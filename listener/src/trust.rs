...
mod tests {
    ...
    fn a_volume_with_no_launcher_is_not_a_cartridge() {
        let dir = std::env::temp_dir().join("gacasy-trust-empty");
    ...
    fn an_unsigned_launcher_is_refused() {
        let dir = std::env::temp_dir().join("gacasy-trust-unsigned");
        let _ = fs::remove_dir_all(&dir);
        fs::create_dir_all(&dir).expect("temp dir");
        fs::write(dir.join(LAUNCHER_NAME), b"MZ but nobody signed it").expect("write");
        assert!(matches!(verify_launcher(&dir), Err(Refusal::Unsigned)));
    }
    ...
    fn a_launcher_signed_by_a_stranger_is_refused() {
        ...
        let dir = std::env::temp_dir().join("gacasy-trust-stranger");
        let _ = fs::remove_dir_all(&dir);
        fs::create_dir_all(&dir).expect("temp dir");
        ...
        assert!(verify_launcher(&dir).is_err());
    }
